...
class Sanitizer:
    ...
    def _remove_chorus(lyrics):
        ...
        subChorus2 = re.sub(r'(\[(Chorus| CHORUS)[^\n]*?\]|Chorus:|Chorus( |\t)*?(\([^\n]*?\)|\{[^\n]*?\}):).*?\[',
                            '[',
                            subChorus,
                            flags=re.DOTALL);  # Catches poorly formatted pages(CHECK IF ALL ARE NEEDED)
        return subChorus2

    @staticmethod
    def _remove_rest(lyrics):
        subSpecialCases = re.sub(
            r'(\[Nfamous\]\nClap your hands|\[John Legend\]\nI lay awake|\[Mike Posner\]\nGirl I\'m on that).*?$',
            '',
            lyrics,
            flags=re.DOTALL);  # Special Case for 1200Tech & (2) 2-Chaiz lyrics
        subNonLyricTags = re.sub(
        ...
        subNonLyricTags2 = re.sub(
            r'(\[(Interlude|Hook|[sS]cratches|Outro|[rR]epeat|Breakdown).*?\]|(Interlude:|Hook:|[sS]cratches:|Outro[^\n]*?:|Breakdown[^\n]*?:)).*?\[',
            '[',
            subNonLyricTags,
            flags=re.DOTALL);  # Catches poorly formatted pages
    ...
    @staticmethod
    def _remove_single_words(lyrics):
        subOneWordLine = re.sub(r'^( |\t)*?\S+?( |\t)*?(\n|$)', '', lyrics, flags=re.MULTILINE)
        return subOneWordLine

    @staticmethod
    def _remove_whitespace(lyrics):
